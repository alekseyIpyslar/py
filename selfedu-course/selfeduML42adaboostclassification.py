...
XN = len(train_data)  # length of training sample
T = 1                 # number of algorithms in composition
max_depth = 2         # maximum depth of decision trees
w = np.ones(XN) / XN  # initial values of weights for sample objects
algs = []             # list of obtained algorithms
alfa = []             # list of calculated weights for composition

for n in range(T):
    # create and train a decision tree with weights of objects w
    algs.append(DecisionTreeClassifier(criterion='gini', max_depth=max_depth))
    algs[n].fit(train_data, train_labels, sample_weight=w)

    predicted = algs[n].predict(train_data) # generate predictions of the obtained tree based on the training sample
    N = np.sum(np.abs(train_labels - predicted) / 2 * w) # calculate the proportion of incorrect classifications
    alfa.append(0.5 * np.log((1 - N) / N) if N != 0 else np.log((1-1e-8) / 1e-8)) # calculate the weight for the current algorithm

    # recalculate the weights of the sample objects
    w = w * np.exp(-1 * alfa[n] * train_labels * predicted)
    w = w / np.sum(w)

# calculate the number of classification errors based on the resulting composition
...
print(f"Number of errors in the training sample: {N} with a composition of {T} decision trees.")

# display the obtained classification results
